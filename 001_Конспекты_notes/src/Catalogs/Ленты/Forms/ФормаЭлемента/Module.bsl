...
Функция ПолучитьТекстДляВставки_Двойной(СтрокаТЧ)
	Если ЗначениеЗаполнено(СтрокаТЧ.Якорь) Тогда
		ПолучитьТекстдляВставки_Якорь(СтрокаТЧ);	
	Иначе
...
Функция ПолучитьТекстДляВставки(СтрокаТЧ)
	...
	МассивСлов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивСлов(СтрокаТЧ.Комментарий, Символы.ПС);
	Если Найти(СтрокаТЧ.Комментарий, Символы.ПС) Тогда
		
	КонецЕсли;
	ТекстДляВставки = СтрЗаменить(СтрокаТЧ.Комментарий,Символы.ПС ,"<br>");	
	Для Каждого СтрокаМассива Из МассивСлов Цикл
	ТекстДляВставки = ТекстДляВставки + "<" + СтрокаТЧ.Тэг.Наименование + " style = 'margin-left: 40px';>" 
		+ СтрокаМассива + "</"
		+ СтрокаТЧ.Тэг.Наименование + ">";
	КонецЦикла;
	
//	ТекстДляВставки = "<" + СтрокаТЧ.Тэг.Наименование + " style = 'margin-left: 40px';>"
...
&НаСервере
Процедура ПолучитьТекстдляВставки_Якорь(СтрокаТЧ)
